...
const generateMemberPDF = (member) => {
    return new Promise((resolve, reject) => {
        try {
            ...
            // -- STYLES --
            const primaryColor = '#2E7D32'; // Green for Nature Farming
            const black = '#000000';
            const grey = '#555555';
            ...
            // Header
            doc.fillColor(primaryColor).fontSize(20).font('Helvetica-Bold').text('Nature Farming', 110, 45);
            doc.fontSize(10).font('Helvetica').fillColor(black)
                .text('Member Registration Summary', 110, 70);

            doc.fillColor(primaryColor).fontSize(24).font('Helvetica-Bold')
                .text('MEMBER PROFILE', 350, 45, { align: 'right', width: 200 });

            doc.moveDown(4);

            // Content Box
            const boxTop = 150;
            doc.rect(40, boxTop, 515, 200).stroke();
            doc.rect(40, boxTop, 515, 30).fill(primaryColor); // Header bar
            doc.fillColor('#FFFFFF').fontSize(14).font('Helvetica-Bold').text('Personal Information', 50, boxTop + 8);
            ...
            const addField = (label, value) => {
                ...
                doc.font('Helvetica-Bold'); // Reset
                y += 30; // Spacing
            };

            addField('Member Code:', member.memberCode);
            addField('Full Name:', member.name);
            addField('NIC Number:', member.nic);
            addField('Mobile:', member.mobile);
            addField('Address:', member.address);
            addField('Email:', member.email);
            addField('Registered Date:', new Date(member.createdAt || Date.now()).toLocaleDateString());

            // Footer
            doc.fontSize(10).font('Helvetica-Oblique').text('Nature Farming - Cultivating the Future', 40, 750, { align: 'center', width: 515 });
...
module.exports = { generateBillPDF, generateMemberPDF };
